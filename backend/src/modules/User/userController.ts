  ...
  if (userObj.role === "advertiser") {
    ...
    if (advertiserProfile) {
      ...
      userObj.profileCompleted =
        advertiserProfile.profileCompleted || userObj.profileCompleted || false;
      userObj.niche = advertiserProfile.niche || userObj.niche || "";
      userObj.contentTypes =
        advertiserProfile.contentFormats || userObj.contentTypes || [];
      userObj.targetAudience = advertiserProfile.targetAudience ||
        userObj.targetAudience || { ageRange: "", gender: "", interests: [] };
      userObj.experienceLevel =
        advertiserProfile.experienceLevel || userObj.experienceLevel || "";
      userObj.profileData =
        advertiserProfile.profileData || userObj.profileData || {};
    }
  } else if (userObj.role === "business_owner") {
    const businessProfile = await BusinessOwner.findOne({
      userId: userObj._id,
    }).lean();
    if (businessProfile) {
      userObj.businessName =
        businessProfile.businessName || userObj.businessName || "";
      userObj.location = businessProfile.location || userObj.location || "";
      userObj.bio = businessProfile.bio || userObj.bio || "";
      userObj.tradeLicenseUrl =
        businessProfile.tradeLicenseUrl || userObj.tradeLicenseUrl || "";
      userObj.idVerificationUrl =
        businessProfile.idVerificationUrl || userObj.idVerificationUrl || "";
      userObj.profileData =
        businessProfile.profileData || userObj.profileData || {};
    }
  }
  return userObj;
}
...
export async function syncBusinessOwnerProfile(user: any) {
  ...
  businessProfile.businessName =
    pData.businessName ??
    pendingPData.businessName ??
    user.businessName ??
    businessProfile.businessName;
  businessProfile.businessEmail =
    pData.businessEmail ??
    pendingPData.businessEmail ??
    user.businessEmail ??
    businessProfile.businessEmail;
  businessProfile.phoneNumber =
    ...
    pendingPData.phone ??
    pData.phoneNumber ??
    pendingPData.phoneNumber ??
    user.phoneNumber ??
    user.phone ??
    businessProfile.phoneNumber;
  businessProfile.website =
    pData.websiteUrl ??
    pendingPData.websiteUrl ??
    pData.website ??
    pendingPData.website ??
    user.websiteUrl ??
    user.website ??
    businessProfile.website;
  businessProfile.industry =
    pData.businessCategory ??
    pendingPData.businessCategory ??
    pData.industry ??
    pendingPData.industry ??
    user.industry ??
    businessProfile.industry;
  businessProfile.companySize =
    pData.companySize ??
    pendingPData.companySize ??
    user.companySize ??
    businessProfile.companySize;
  businessProfile.location =
    pData.businessLocation ??
    pendingPData.businessLocation ??
    user.location ??
    businessProfile.location;
  businessProfile.bio =
    pData.brandDescription ??
    pendingPData.brandDescription ??
    user.bio ??
    businessProfile.bio;
  businessProfile.tradeLicenseUrl =
    pData.tradeLicenseUrl ??
    pendingPData.tradeLicenseUrl ??
    user.tradeLicenseUrl ??
    businessProfile.tradeLicenseUrl;
  businessProfile.idVerificationUrl =
    pData.idVerificationUrl ??
    pendingPData.idVerificationUrl ??
    user.idVerificationUrl ??
    businessProfile.idVerificationUrl;

  // Sync structural objects
  businessProfile.profileData = user.profileData || businessProfile.profileData;
  businessProfile.pendingProfileData =
    user.pendingProfileData || businessProfile.pendingProfileData;
  businessProfile.pendingUpdates =
    user.pendingUpdates || businessProfile.pendingUpdates;
  ...
  try {
    ...
    if (user.role === "advertiser") {
      ...
      const applyAdvertiserProfileMerge = () => {
        return mergeAdvertiserProfileOnSubmit({
          profileData: (req.body.profileData ?? user.profileData) as Record<
            string,
            unknown
          >,
      ...
      // Sync fields
      advertiserProfile.niche =
        req.body.niche ??
        advertiserProfile.niche ??
        (user.profileData as any)?.niche;
      advertiserProfile.experienceLevel =
        req.body.experienceLevel ??
        advertiserProfile.experienceLevel ??
        (user.profileData as any)?.experienceLevel;
      advertiserProfile.contentFormats =
        req.body.contentTypes ??
        advertiserProfile.contentFormats ??
        (user.profileData as any)?.contentTypes;
      advertiserProfile.targetAudience =
        req.body.targetAudience ??
        advertiserProfile.targetAudience ??
        (user.profileData as any)?.targetAudience;
      ...
      if (user.connectedAccounts) {
        hasConnected = ["tiktok", "instagram", "facebook"].some(
          (p) =>
            ...
            user.connectedAccounts[p].connected &&
            user.connectedAccounts[p].verified,
        );
    ...
    const hasVerifiedSocial =
      (user.socialProfiles &&
        user.socialProfiles.some(
          (p: any) => p.platform?.toLowerCase() === "tiktok" && p.verified,
        )) ||
      (req.body.socialProfiles &&
        req.body.socialProfiles.some(
          (p: any) => p.platform?.toLowerCase() === "tiktok" && p.verified,
        ));

    const isAutomaticallyVerified =
      user.role === "advertiser" && hasVerifiedSocial;
    ...
    if (isAutomaticallyVerified && !isAlreadyApproved) {
      ...
      if (mergedAdv) {
        ...
        updates.profileData &&
        typeof updates.profileData === "object"
      ) {
        updates.profileData = mergeProfileData(
          (user.profileData || {}) as Record<string, unknown>,
          updates.profileData as Record<string, unknown>,
      ...
      if (mergedAdv) {
        ...
        if (mergedAdv.socialProfiles?.length) {
          user.socialProfiles =
            mergedAdv.socialProfiles as typeof user.socialProfiles;
        ...
        if (req.body.profileData && typeof req.body.profileData === "object") {
          businessProfile.pendingProfileData = mergeProfileData(
            mergeProfileData(
              ...
              (businessProfile.pendingProfileData || {}) as Record<
                string,
                unknown
              >,
            ),
...
export const syncUser = async (req: Request, res: Response): Promise<void> => {
  ...
  try {
    ...
    if (userByEmail) {
      console.log(
        `[syncUser] Email ${email} exists with different clerkId. Updating clerkId.`,
      );
    ...
    if (platformSettings.allowPublicSignup === false) {
      res
        .status(403)
        .json({ message: "New account registration is temporarily disabled." });
      return;
    }
    ...
    const resolvedRole =
      clerkRole &&
      ["super_admin", "admin", "business_owner", "advertiser"].includes(
        clerkRole,
      )
        ? clerkRole
    ...
    try {
      ...
      const startingCoins = Math.max(
        0,
        Math.round(platformSettings.newUserStartingCoins ?? 1000),
      );
      ...
      try {
        ...
        console.log(
          `[syncUser] Credited ${startingCoins} starting coins to ${user.email}`,
        );
      } catch (walletError) {
        console.error(
          `[syncUser] Failed to credit coins for ${user.email}:`,
          walletError,
        );
      ...
      res
        .status(201)
        .json({ user: enriched, message: "User created Successfully" });
...
export const toggleSavedOpportunity = async (
  ...
  try {
    ...
    if (user.role !== "advertiser") {
      res
        .status(403)
        .json({ message: "Only advertisers can save opportunities" });
      return;
    }
    ...
    const isSaved = advertiserProfile.savedOpportunities.some(
      (id: any) => id.toString() === opportunityId.toString(),
    );

    if (isSaved) {
      // Unsave
      advertiserProfile.savedOpportunities =
        advertiserProfile.savedOpportunities.filter(
...
export const toggleSavedCreator = async (
  ...
  try {
    ...
    const isSaved = user.savedCreators.some(
      (id: any) => id.toString() === creatorId.toString(),
    );

    if (isSaved) {
      user.savedCreators = user.savedCreators.filter(
        (id: any) => id.toString() !== creatorId.toString(),
      );
    } else {
    ...
    res.status(200).json({
      message: isSaved
        ? "Creator removed from bookmarks"
        : "Creator bookmarked successfully",
      isSaved: !isSaved,
...
export const completeAdvertiserProfile = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
...
export const syncTikTokMetrics = async (
  req: Request,
  res: Response,
): Promise<void> => {
  try {
    ...
    if (!username) {
      res
        ...
        .json({
          success: false,
          message: "No TikTok username connected to this account",
        });
      return;
    }
    ...
    if (!apifyToken) {
      res
        ...
        .json({
          success: false,
          message: "APIFY_TOKEN not configured on server",
        });
      return;
    }
    ...
    if (!items || items.length === 0) {
      res
        ...
        .json({
          success: false,
          message: "TikTok profile not found or private.",
        });
      return;
    }
    ...
    if (userData.error) {
      res
        ...
        .json({
          success: false,
          message: `TikTok profile error: ${userData.error}`,
        });
      return;
    }
    ...
    user.tiktokProfile = {
      displayName:
        userData?.authorMeta?.nickName ||
        user.tiktokProfile?.displayName ||
        username,
      bio: userData?.authorMeta?.bio || user.tiktokProfile?.bio || "",
      profilePicture:
        userData?.authorMeta?.avatar ||
        user.tiktokProfile?.profilePicture ||
        "",
    ...
    user.nextVerificationRequiredAt = new Date(
    ...
    res
      ...
      .json({
        success: false,
        message: "Failed to sync TikTok metrics",
        error: error.message,
      });
  }
};
 ...
 * Returns the calling user's own trust score.
 */
export const getMyTrustScore = async (
  ...
  try {
    ...
    const data = await adminAnalyticsService.calculateTrustScore(userId);
