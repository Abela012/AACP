...
export const applyToOpportunity = async (req: Request, res: Response) => {
  try {
    ...
    if (!user || !user._id) {
      return error(
        res,
        "User information not found. Please sync your profile.",
        401,
      );
    }
    ...
    try {
      ...
      console.log(
        `[Apply] User ${advertiserId} balance: ${balanceInfo.availableBalance} available / ${balanceInfo.balance} total`,
      );
    } catch (balanceErr: any) {
      console.error(
        `[Apply] Balance check failed for ${advertiserId}:`,
        balanceErr.message,
      );
    ...
    if (balanceInfo.availableBalance < 50) {
      console.log(
        `[Apply] Insufficient coins: ${balanceInfo.availableBalance} < 50`,
      );
      return error(
        res,
        `Insufficient coins. You have ${balanceInfo.availableBalance} coins but need 50 to apply.`,
        400,
      );
    }
    ...
    console.log(
      `[Apply] Submitting application: opportunity=${data.opportunity}, advertiser=${advertiserId}`,
    );
...
export const getApplicationsByOpportunity = async (
  ...
  try {
    const applications = await applicationService.getApplicationsByOpportunity(
      req.params.id as string,
    );
 ...
 * @access  Private
 */
export const getApplicationsByAdvertiser = async (
  req: Request,
  res: Response,
) => {
  try {
    const applications = await applicationService.getApplicationsByAdvertiser(
      req.params.id as string,
    );
    return success(
      res,
 ...
 * @access  Private
 */
export const getApplicationsForBusinessOwner = async (
  req: Request,
  res: Response,
) => {
  try {
    ...
    const applications =
      await applicationService.getApplicationsForBusinessOwner(userId);
    return success(
      res,
      "Business owner applications retrieved successfully",
      applications,
    );
  } catch (err: any) {
    return error(res, err.message, 500);
...
export const acceptApplication = async (req: Request, res: Response) => {
  try {
    ...
    try {
      ...
      collaboration = await collaborationService.startCollaboration(
        req.params.id as string,
        userId,
      );
      console.log(
        `[Accept] Collaboration workspace auto-created: ${collaboration._id}`,
      );
    } catch (collabErr: any) {
      console.error(
        "[Accept] Failed to auto-create collaboration:",
        collabErr.message,
      );
    ...
    if (io && application.advertiser) {
      io.to(`user:${application.advertiser.toString()}`).emit(
        "notification:new",
        {
          type: "application",
