...
export const startCollaboration = async (
  ...
  if (
    application.opportunity.businessOwner.toString() !==
 ...
 */
export const getCollaborationsByUser = async (
  userId: string,
): Promise<ICollaboration[]> => {
  let mongoId = userId;
  ...
  const collaborations = await Collaboration.find({
    $or: [{ businessOwner: mongoId }, { advertiser: mongoId }],
  })
...
export const updateTaskStatus = async (
  ...
  if (
    status === "approved" &&
    collaboration.businessOwner.toString() !== userId
  ) {
...
/**
 * Report a partner in a collaboration — creates a Dispute record visible to admins
 */
export const reportPartner = async (
  ...
  const isBusinessOwner = collaboration.businessOwner.toString() === reporterId;
  const isAdvertiser = collaboration.advertiser.toString() === reporterId;

  if (!isBusinessOwner && !isAdvertiser) {
    throw new Error("You are not a participant in this collaboration");
  }

  const againstId = isBusinessOwner
    ? collaboration.advertiser.toString()
    : collaboration.businessOwner.toString();

  const count = await Dispute.countDocuments();
  const disputeId = `DISP-${1000 + count + 1}`;
  ...
  const dispute = await Dispute.create({
    ...
    metadata: {
      ...
      reportedBy: isBusinessOwner ? "business_owner" : "advertiser",
    },
  });
