...
export const SocialConnectionModal: React.FC<SocialConnectionModalProps> = ({ platform, onClose, onSuccess }) => {
    ...
    const handleInitiate = async () => {
        ...
        try {
            const res = await socialApi.initiateConnection(api, platform, username);
            ...
            setErrorMsg(err.response?.data?.message || err.message || 'Failed to initiate verification');
        } finally {
    ...
    const handleVerify = async () => {
        ...
        setIsVerifying(true);
        try {
            const res = await socialApi.verifyConnection(api, platform, username, verificationCode);
            if (res.success) {
                setState('success');
                refreshProfile();
            ...
            setState('verifying');
            setErrorMsg(err.response?.data?.message || err.message || 'Verification failed. Make sure you posted the code.');
        } finally {
    ...
    const platformIcon = platform === 'tiktok' ? <TikTokIcon /> : platform === 'instagram' ? <FaInstagram size={24} /> : <FaFacebook size={24} />;
        ...
        <div className="fixed inset-0 z-100 flex items-center justify-center p-4 bg-black/60 backdrop-blur-sm">
            ...
            >
                ...
                <div className="flex justify-center mb-6 text-black dark:text-white">
                    <div className={`w-14 h-14 rounded-2xl flex items-center justify-center bg-gray-100 dark:bg-white/10`}>
                        {platformIcon}
                    </div>
                </div>
                ...
                {state === 'input' && (
                    <>
                        <h3 className="text-xl font-bold text-center mb-2 dark:text-white">Connect {platformName}</h3>
                        <p className="text-sm text-center text-gray-500 mb-8">Enter your {platformName} username to verify ownership and connect it to your profile.</p>
                        ...
                        <button
                            ...
                            disabled={isInitiating || !username}
                        ...
                        >
                            {isInitiating ? <Loader2 size={18} className="animate-spin" /> : 'Continue'}
                        </button>
                    </>
                )}
                ...
                {state === 'verifying' && (
                    <>
                        ...
                        <p className="text-sm text-center text-gray-500 mb-6">
                            Paste this code into your <b>{platformName} bio</b>.
                        </p>
                        ...
                        <div className="flex items-center justify-between text-xs text-gray-500 font-bold px-2">
                            <span>Expires in: <span className={timeLeft === 'Expired' ? 'text-red-500' : 'text-emerald-500'}>{timeLeft}</span></span>
                        </div>
                    </>
                )}
                ...
                {state === 'checking' && (
                    <div className="py-8 flex flex-col items-center justify-center text-center">
                        ...
                        <h3 className="text-xl font-bold mb-2 dark:text-white">Scraping {platformName}...</h3>
                        <p className="text-sm text-gray-500">Looking for your code in your bio. This takes about 10-15 seconds.</p>
                    </div>
                )}
                ...
                {state === 'success' && (
                    <div className="py-8 flex flex-col items-center justify-center text-center">
                        ...
                        <p className="text-sm text-gray-500">Your {platformName} account has been connected.</p>
                    </div>
                )}
            </motion.div>
        </div>
