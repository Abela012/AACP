...
export default function LandingPage() {
  ...
  return (
    <div className="min-h-screen bg-white text-[#001e00] font-sans selection:bg-[#14a800]/20 selection:text-[#001e00]">
      ...
      <section id="testimonials" className="py-24 bg-[#f7faf7]">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          {/* Section Header */}
          <motion.div
            initial="hidden"
          ...
          >
            <motion.p variants={fadeInUp} className="text-sm font-bold text-[#14a800] uppercase tracking-widest mb-3">
              Success Stories
            </motion.p>
            <motion.h2 variants={fadeInUp} className="text-4xl sm:text-5xl font-bold text-[#001e00] mb-5">
              Trusted by creators & businesses
            </motion.h2>
            <motion.p variants={fadeInUp} className="text-lg text-[#5e6d55] max-w-2xl mx-auto">
              See how professionals on AACP are growing their reach, landing campaigns, and building lasting partnerships.
            </motion.p>
          </motion.div>
          ...
          <motion.div
            ...
            viewport={{ once: true }}
            variants={fadeInUp}
            className="mt-16 bg-white rounded-2xl border border-gray-100 p-8 flex flex-col sm:flex-row items-center justify-around gap-8"
          >
            {[
              { value: '2,500+', label: 'Active Creators' },
              { value: '850+', label: 'Business Partners' },
              { value: '12,000+', label: 'Campaigns Completed' },
              { value: '4.9/5', label: 'Average Rating' },
            ].map((stat, i) => (
              <div key={i} className="text-center">
                <p className="text-3xl font-bold text-[#14a800]">{stat.value}</p>
                <p className="text-xs text-[#5e6d55] uppercase tracking-widest font-bold mt-1">{stat.label}</p>
              </div>
            ))}
          </motion.div>
        </div>
