...
export default function AdminHelpPage() {
  const categories = [
    ...
    { title: 'Dispute Resolution', icon: MessageSquare, desc: 'Handling complaints and three-way chats.', color: 'text-primary-blue', bg: 'bg-neutral-border/15 dark:bg-primary-blue/10' },
    { title: 'Platform Policy', icon: FileText, desc: 'Community guidelines and legal docs.', color: 'text-primary-blue', bg: 'bg-neutral-border/15 dark:bg-primary-blue/10' },
  ];
  ...
  return (
    <AdminLayout>
      <div className="max-w-[1000px] mx-auto pb-12">
        {/* Hero Section */}
        <div className="bg-linear-to-tr from-primary-blue to-neutral-border rounded-[3rem] p-12 text-white mb-12 shadow-xl shadow-neutral-border/25 dark:shadow-none relative overflow-hidden">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8 mb-12">
          {categories.map((cat, i) => (
            ...
            >
              ...
              <h3 className="text-lg font-black mb-2 group-hover:text-primary-blue transition-colors">{cat.title}</h3>
              <p className="text-sm font-medium text-[#6F767E] dark:text-gray-400 mb-6 leading-relaxed">{cat.desc}</p>
              <div className="flex items-center gap-2 text-[10px] font-black text-primary-blue uppercase tracking-widest">
        ...
        <div className="bg-white dark:bg-[#111111] rounded-[3rem] border border-[#EFEFEF] dark:border-white/5 shadow-sm overflow-hidden">
          <div className="p-8 border-b border-[#F4F4F4] dark:border-white/5 flex justify-between items-center">
            ...
            <button className="text-[10px] font-black text-primary-blue uppercase tracking-widest hover:underline">View all FAQ</button>
          </div>
          <div className="divide-y divide-[#F4F4F4] dark:divide-white/5">
            {faqs.map((faq, i) => (
              <button key={i} className="w-full p-6 text-left hover:bg-[#F8F8FD] dark:hover:bg-white/5 transition-all flex items-center justify-between group">
                <span className="text-sm font-bold text-[#1A1D1F] dark:text-white group-hover:text-primary-blue transition-colors">{faq}</span>
                <ExternalLink size={14} className="text-[#9A9FA5] group-hover:text-primary-blue transition-colors" />
              </button>
            ))}
          </div>
        </div>

        <div className="mt-12 text-center p-8 bg-neutral-border/20 dark:bg-primary-blue/5 rounded-[2.5rem] border border-neutral-border/25 dark:border-primary-blue/10">
          <p className="text-sm font-bold text-primary-blue mb-4">Still need help?</p>
          <h3 className="text-xl font-black mb-6">Contact System Support</h3>
          <button className="px-8 py-4 bg-primary-blue text-white rounded-2xl font-black text-xs uppercase tracking-widest shadow-lg shadow-neutral-border/25 dark:shadow-none hover:bg-primary-blue transition-all">
            Open Support Ticket
          </button>
        </div>
      </div>
