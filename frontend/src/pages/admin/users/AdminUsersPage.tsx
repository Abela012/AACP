...
export default function AdminUsersPage() {
  ...
  const statusStyle = (status: AdminUser['status']) => {
    if (status === 'active' || status === 'approved') return 'bg-neutral-border/25 dark:bg-primary-blue/20 text-primary-blue';
          ...
          <div>
            ...
            <p className="text-sm font-medium text-[#6F767E] dark:text-gray-400">
              ...
              {data?.total !== undefined && (
                <span className="ml-2 text-primary-blue font-bold">{data.total.toLocaleString()} users total</span>
              )}
            </p>
          </div>
              ...
              {['All', 'Advertiser', 'Business Owner'].map((role) => (
                <button
                  ...
                  className={`px-4 py-2 text-[10px] font-black uppercase tracking-widest rounded-xl transition-all ${
                    activeFilter === role
                      ? 'bg-white dark:bg-white/10 shadow-sm text-primary-blue'
                          ...
                          <div className="flex justify-end gap-2">
                            ...
                            {(!user.isVerified || user.pendingProfileData || user.pendingUpdates || user.status === 'pending') && (
                              <Link
                                ...
                                className="p-2 hover:bg-neutral-border/15 dark:hover:bg-primary-blue/10 rounded-xl transition-all text-primary-blue"
                            ...
                            <button
                              ...
                              className={`p-2 hover:bg-white dark:hover:bg-white/10 rounded-xl transition-all disabled:opacity-50 ${
                                user.status === 'banned' ? 'text-primary-blue' : 'text-[#9A9FA5] hover:text-red-500'
                              }`}
