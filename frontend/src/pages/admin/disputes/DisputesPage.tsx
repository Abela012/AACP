...
import {
  ...
  useResolveDispute,
  useEscalateDispute,
} from "@/src/hooks/useAdminDisputes";
...
export default function DisputesPage() {
  ...
  const { data: disputesData, isLoading } = useAdminDisputes({
  ...
  const stats = [
    {
      label: "Open Disputes",
      value: disputes.filter((d: any) => d.status === "OPEN").length.toString(),
      icon: AlertCircle,
    ...
    },
    {
      label: "Under Review",
      value: disputes
        .filter((d: any) => d.status === "UNDER REVIEW")
        .length.toString(),
      icon: Scale,
    ...
    },
    {
      label: "Escalated",
      value: disputes
        .filter((d: any) => d.status === "ESCALATED")
        .length.toString(),
      icon: ShieldAlert,
  ...
  const tabs = [
    ...
    },
    {
      ...
      label: "Resolved",
      count: disputes.filter((d: any) => d.status === "RESOLVED").length,
    },
    {
      key: "escalated" as const,
      label: "Escalated",
      count: disputes.filter((d: any) => d.status === "ESCALATED").length,
    },
  ];
  ...
  const filteredDisputes = disputes.filter((d: any) => {
    ...
    let matchesTab = true;
    if (activeTab === "open")
      matchesTab = d.status === "OPEN" || d.status === "UNDER REVIEW";
  ...
  const getStatusStyle = (status: string) => {
    switch (status) {
      ...
      case "UNDER REVIEW":
        return "bg-blue-100 dark:bg-blue-500/20 text-blue-600";
      case "RESOLVED":
        return "bg-neutral-border/25 dark:bg-primary-blue/20 text-primary-blue";
      case "ESCALATED":
        return "bg-red-100 dark:bg-red-500/20 text-red-600";
      default:
      ...
      <div className="max-w-7xl mx-auto pb-12">
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center gap-6 mb-10">
          <div>
            ...
            <p className="text-sm font-medium text-[#6F767E] dark:text-gray-400">
              Review and resolve platform disputes, complaints, and escalations.
            </p>
          </div>
                ...
                {showExportMenu && (
                  ...
                  >
                    <div className="p-2">
                      {[
                        {
                          label: "Case Summary (PDF)",
                          format: "PDF",
                          icon: "📄",
                        },
                        {
                          label: "Evidence Logs (CSV)",
                          format: "CSV",
                          icon: "📊",
                        },
                        {
                          label: "Resolution Data (XLS)",
                          format: "Excel",
                          icon: "📑",
                        },
                      ].map((item) => (
                        ...
                        >
                          ...
                          <div>
                            <p className="text-xs font-bold text-[#1A1D1F] dark:text-white leading-none mb-1">
                              {item.format}
                            </p>
                            <p className="text-[10px] text-[#6F767E] font-medium leading-none">
                              {item.label}
                            </p>
                          </div>
                        </button>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-10">
          {stats.map((stat, idx) => (
            ...
            >
              <div
                className={`w-14 h-14 rounded-2xl flex items-center justify-center ${stat.bg} ${stat.iconColor}`}
              >
                <stat.icon size={24} />
              </div>
              <div>
                ...
                <span className="text-2xl font-black text-[#1A1D1F] dark:text-white">
                  {stat.value}
                </span>
              </div>
            </motion.div>
            ...
            <table className="w-full">
              <thead>
                <tr className="text-left border-b border-[#F4F4F4] dark:border-white/5">
                  <th className="py-6 px-8 text-[10px] font-black text-[#9A9FA5] uppercase tracking-widest">
                    Dispute
                  </th>
                  <th className="py-6 px-8 text-[10px] font-black text-[#9A9FA5] uppercase tracking-widest">
                    Reporter
                  ...
                  <th className="py-6 px-8 text-[10px] font-black text-[#9A9FA5] uppercase tracking-widest text-right">
                    Actions
                  </th>
                </tr>
              </thead>
                ...
                {isLoading ? (
                  ...
                  <tr>
                    <td colSpan={6} className="py-20 text-center">
                      ...
                      <h3 className="text-sm font-bold text-[#1A1D1F] dark:text-white mb-1">
                        No disputes found
                      </h3>
                    ...
                    >
                      <td className="py-6 px-8">
                        <div className="flex items-start gap-3">
                          <div
                            className={`w-2 h-2 rounded-full mt-2 shrink-0 ${getPriorityStyle(dispute.priority)}`}
                            style={{ backgroundColor: "currentColor" }}
                          />
                          <div>
                            <p className="text-xs font-black text-[#1A1D1F] dark:text-white mb-1">
                              {dispute.id}
                            </p>
                            <p className="text-xs font-medium text-[#6F767E] dark:text-gray-400 leading-tight max-w-50">
                              {dispute.title}
                            </p>
                            <p className="text-[10px] font-medium text-[#9A9FA5] mt-1">
                              {dispute.date}
                            </p>
                          </div>
                        </div>
                      ...
                      <td className="py-6 px-8">
                        <div className="flex items-center gap-3">
                          <div className="w-8 h-8 rounded-lg overflow-hidden bg-gray-100 shrink-0">
                            <img
                              src={dispute.reporterAvatar}
                              alt={dispute.reporter}
                          ...
                          <div>
                            ...
                            <p className="text-[10px] font-bold text-[#9A9FA5] uppercase">
                      ...
                      <td className="py-6 px-8">
                        <p className="text-sm font-black text-[#1A1D1F] dark:text-white">
                          {dispute.amount}
                        </p>
                      ...
                      <td className="py-6 px-8">
                        <span
                          className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${getStatusStyle(dispute.status)}`}
                        >
                          {dispute.status}
                        </span>
                      </td>
                        ...
                        <div className="flex justify-end gap-2">
                          ...
                          {dispute.status !== "ESCALATED" &&
                            dispute.status !== "RESOLVED" && (
                              <button
                                onClick={() =>
                                  handleAction(dispute.id, "escalate")
                                }
                          ...
                          {dispute.status !== "RESOLVED" && (
                            <button
                              onClick={() =>
                                handleAction(dispute.id, "resolve")
                              }
                              className="p-2 hover:bg-white dark:hover:bg-white/10 rounded-xl transition-all text-[#9A9FA5] hover:text-primary-blue"
                            >
                              <CheckSquare size={18} />
              ...
              <div className="p-8">
                <div className="flex justify-between items-center mb-8">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <h2 className="text-lg font-black text-[#1A1D1F] dark:text-white leading-none mb-1">
                        Case Detail
                      </h2>
                      <p className="text-xs font-bold text-[#9A9FA5] uppercase tracking-widest">
                        {selectedDispute.id}
                      </p>
                    </div>
                  </div>
                ...
                <div className="space-y-8">
                  ...
                  <div className="flex gap-4">
                    <div className="flex-1 bg-gray-50 dark:bg-white/5 p-4 rounded-2xl border border-[#EFEFEF] dark:border-white/10">
                      ...
                      <span
                        className={`px-3 py-1 rounded-lg text-[10px] font-black uppercase tracking-widest ${getStatusStyle(selectedDispute.status)}`}
                      >
                    ...
                    <div className="flex-1 bg-gray-50 dark:bg-white/5 p-4 rounded-2xl border border-[#EFEFEF] dark:border-white/10">
                      ...
                      <div className="flex items-center gap-2">
                        <div
                          className={`w-2 h-2 rounded-full ${getPriorityStyle(selectedDispute.priority)}`}
                          style={{ backgroundColor: "currentColor" }}
                        />
                        <span
                          className={`text-[10px] font-black uppercase tracking-widest ${getPriorityStyle(selectedDispute.priority)}`}
                        >
                  ...
                  {/* Dispute Content */}
                  <div>
                    <h3 className="text-sm font-black text-[#1A1D1F] dark:text-white mb-3">
                      Dispute Description
                    </h3>
                    <div className="bg-[#F8F8FD] dark:bg-white/5 p-5 rounded-2xl border border-blue-50 dark:border-white/10">
                      <p className="text-sm font-medium text-[#1A1D1F] dark:text-gray-300 leading-relaxed">
                        {selectedDispute.title}. The user reports that the
                        system failed to acknowledge their campaign deliverables
                        despite multiple verification attempts.
                      </p>
                    </div>
                  </div>
                  ...
                  {/* Parties Involved */}
                  <div>
                    <h3 className="text-sm font-black text-[#1A1D1F] dark:text-white mb-4">
                      Parties Involved
                    </h3>
                    <div className="space-y-3">
                      <div className="flex items-center justify-between p-4 bg-white dark:bg-white/5 rounded-2xl border border-[#EFEFEF] dark:border-white/10">
                        <div className="flex items-center gap-3">
                          <img
                            src={selectedDispute.reporterAvatar}
                          ...
                          <div>
                            ...
                            <p className="text-[10px] font-bold text-[#9A9FA5]">
                              REPORTER ({selectedDispute.reporterRole})
                            </p>
                          </div>
                        </div>
                        <Link
                          to={`/admin/users/${selectedDispute.id}`}
                          className="p-2 text-[#9A9FA5] hover:text-primary-blue"
                        >
                          <ArrowRight size={18} />
                        </Link>
                      </div>
                      <div className="flex items-center justify-between p-4 bg-white dark:bg-white/5 rounded-2xl border border-[#EFEFEF] dark:border-white/10">
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            ...
                            <p className="text-[10px] font-bold text-[#9A9FA5]">
                              AGAINST ({selectedDispute.category})
                            </p>
                          </div>
                        </div>
                      </div>
                  ...
                  <div>
                    <div className="flex justify-between items-center mb-4">
                      <h3 className="text-sm font-black text-[#1A1D1F] dark:text-white">
                        Case Timeline
                      </h3>
                      <button className="text-[10px] font-black text-primary-blue uppercase tracking-widest">
                        Full Log
                      </button>
                    </div>
                    <div className="space-y-4 relative before:absolute before:left-5 before:top-2 before:bottom-2 before:w-0.5 before:bg-[#EFEFEF] dark:before:bg-white/10">
                      {[
                        {
                          time: "10:45 AM",
                          event: "Dispute opened by reporter",
                          icon: AlertCircle,
                          color: "text-amber-500",
                        },
                        {
                          time: "11:20 AM",
                          event: "Evidence documentation uploaded",
                          icon: FileText,
                          color: "text-blue-500",
                        },
                        {
                          time: "02:15 PM",
                          event: "System verification triggered",
                          icon: Clock,
                        ...
                        <div key={i} className="flex gap-6 relative">
                          <div
                            className={`w-10 h-10 rounded-xl bg-white dark:bg-[#1A1D1F] border border-[#EFEFEF] dark:border-white/10 flex items-center justify-center z-10 ${item.color}`}
                          >
                            <item.icon size={18} />
                          </div>
                          <div>
                            ...
                            <p className="text-[10px] text-[#9A9FA5] font-medium">
                              {item.time} • {selectedDispute.date}
                            </p>
                          </div>
                        </div>
                      ))}
                  ...
                  <div className="pt-6 border-t border-[#EFEFEF] dark:border-white/10 flex gap-3">
                    {selectedDispute.status !== "RESOLVED" && (
                      <button
                        onClick={() =>
                          handleAction(selectedDispute.id, "resolve")
                        }
                    ...
                    {selectedDispute.status !== "ESCALATED" &&
                      selectedDispute.status !== "RESOLVED" && (
                        <button
                          onClick={() =>
                            handleAction(selectedDispute.id, "escalate")
                          }
