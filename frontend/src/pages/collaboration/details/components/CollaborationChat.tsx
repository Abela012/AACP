         ...
         <div className="relative flex items-center gap-3">
            ...
            <div className="relative flex-1">
              <input 
                ...
                className="w-full bg-white dark:bg-white/5 border border-gray-100 dark:border-white/10 rounded-2xl px-5 py-4 text-sm focus:outline-none focus:border-emerald-500 pr-12 shadow-inner"
                value={inputText}
                onChange={(e) => setInputText(e.target.value)}
                onKeyDown={(e) => e.key === 'Enter' && handleSend()}
              />
              <button className="absolute right-4 top-1/2 -translate-y-1/2 text-gray-300 hover:text-emerald-500 transition-colors">
                 <Smile size={20} />
              </button>
            </div>
            <button 
              onClick={handleSend}
              className="p-4 bg-emerald-600 text-white rounded-2xl hover:bg-emerald-500 transition-all shadow-lg shadow-emerald-500/20"
            >
               <Send size={20} />
            </button>
         </div>
