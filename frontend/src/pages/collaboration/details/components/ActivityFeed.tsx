...
export const ActivityFeed: React.FC<{ activities: ActivityItem[] }> = ({ activities }) => {
  const getIcon = (type: string) => {
    switch (type) {
      case 'acceptance': return <UserCheck className="text-aacp-olive" size={16} />;
      ...
      case 'task_approved': return <CheckCircle2 className="text-aacp-olive" size={16} />;
      case 'deliverable_uploaded': return <Upload className="text-purple-500" size={16} />;
      case 'revision_requested': return <RefreshCw className="text-amber-500" size={16} />;
      case 'payment_released': return <DollarSign className="text-aacp-olive" size={16} />;
  ...
  return (
    <div className="bg-white dark:bg-[#0a0a0a] rounded-[2.5rem] border border-gray-100 dark:border-white/5 p-8 shadow-sm h-full overflow-hidden flex flex-col">
      <h3 className="text-lg font-black text-gray-900 dark:text-white mb-8 flex items-center gap-2">
        <RefreshCw className="text-aacp-olive animate-spin-slow" size={20} />
        Live Activity Feed
      </h3>
