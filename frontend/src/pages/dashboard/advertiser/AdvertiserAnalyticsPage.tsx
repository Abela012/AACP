...
// ─── Status Badge ──────────────────────────────────────────────────────────────
const StatusBadge: React.FC<{ status: AnalyticsRecord["status"] }> = ({
  status,
}) => {
  const map = {
    pending: {
    ...
    completed: {
      ...
      cls: "bg-primary-blue/10 text-primary-blue",
    },
    failed: {
      label: "Failed",
  ...
  return (
    <span
      className={cn(
        "inline-flex items-center gap-1 text-[9px] font-black uppercase tracking-wide px-2 py-0.5 rounded-full",
        cls,
      )}
    >
      {icon}
...
const MetricChip: React.FC<{
  ...
  <div className="flex flex-col items-center text-center min-w-13">
    <span className={cn("mb-0.5", color || "text-gray-400 dark:text-gray-500")}>
      {icon}
    </span>
...
const PostCard: React.FC<PostCardProps> = ({
  ...
  return (
    <motion.div
      ...
      className="bg-white dark:bg-[#0d0d0d] rounded-3xl border border-gray-100 dark:border-white/5 overflow-hidden hover:border-primary-blue/20 dark:hover:border-primary-blue/10 transition-all group"
    >
      {/* Top bar accent */}
      ...
      <div className="p-5">
        ...
        <div className="flex items-start justify-between gap-3 mb-4">
          <div className="flex items-center gap-3">
            <div
              className={cn(
                "w-11 h-11 rounded-2xl flex items-center justify-center shrink-0 border border-gray-100 dark:border-white/10",
                cfg.bg,
                cfg.color,
              )}
            >
            ...
            <div className="min-w-0">
              ...
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="text-[11px] text-primary-blue hover:underline flex items-center gap-1 truncate max-w-65"
              >
                {record.postUrl.length > 52
                  ? record.postUrl.slice(0, 52) + "…"
                  : record.postUrl}
                <ExternalLink size={9} />
              </a>
              {record.notes && (
                <p className="text-[10px] text-gray-400 mt-0.5 italic truncate max-w-60">
                  "{record.notes}"
                </p>
              )}
              {record.status === "failed" && (
                <p className="text-[10px] text-red-400 mt-0.5 flex items-center gap-1">
                  ...
                  {record.errorMessage ?? "Scraping failed"}
                </p>
              )}
            </div>
          </div>
        ...
        {record.status === "completed" ? (
          <>
            <div className="grid grid-cols-4 gap-2 bg-gray-50/70 dark:bg-white/3 rounded-2xl p-4 border border-gray-100 dark:border-white/5">
              <MetricChip
                icon={<Eye size={12} />}
                label="Views"
                value={record.metrics.views}
                color="text-primary-blue"
              />
              <MetricChip
                icon={<Heart size={12} />}
                label="Likes"
                value={record.metrics.likes}
                color="text-pink-500"
              />
              <MetricChip
                icon={<MessageCircle size={12} />}
                label="Comments"
                value={record.metrics.comments}
                color="text-amber-500"
              />
              <MetricChip
                icon={<Share2 size={12} />}
                label="Shares"
                value={record.metrics.shares}
                color="text-cyan-500"
              />
            </div>
            ...
            <p className="text-[9px] text-gray-400 mt-2.5 flex items-center gap-1">
              ...
              {record.metrics.duration && (
                <>
                  {" "}
                  · {Math.floor(record.metrics.duration / 60)}m{" "}
                  {record.metrics.duration % 60}s
                </>
              )}
            </p>
          </>
...
export default function AdvertiserAnalyticsPage() {
  ...
  const { data: allAnalytics = [], isLoading: isLoadingAnalytics } = useQuery<
    AnalyticsRecord[]
  >({
    ...
    queryFn: async () => {
      ...
      const promises = collabs.map((collab: any) =>
        api
          .get(`/collaborations/${collab._id}/analytics`)
  ...
  const totalEarnings = collaborations.reduce(
    (s: number, c: any) => s + (c.agreedBudget?.amount || 0),
    0,
  );
  ...
  const platformData = [
    {
      name: "TikTok",
      value: Math.round(
        ((platformCounts["TikTok"] || 0) / platformTotal) * 100,
      ),
      color: "#0070BB",
    },
    {
      name: "Instagram",
      value: Math.round(
        ((platformCounts["Instagram"] || 0) / platformTotal) * 100,
      ),
      color: "#ec4899",
    },
    {
      name: "YouTube",
      value: Math.round(
        ((platformCounts["YouTube"] || 0) / platformTotal) * 100,
      ),
      color: "#ef4444",
    },
  ];
  ...
  const chartData = useMemo(() => {
    ...
    if (!sorted.length)
      return days.map((name) => ({ name, views: 0, engagement: 0 }));
    ...
    while (slice.length < 7)
      slice.push({ name: days[slice.length], views: 0, engagement: 0 });
    return slice;
  }, [completed]);
  ...
  const filteredPosts =
    platformFilter === "All"
      ? allAnalytics
      : allAnalytics.filter((r) => r.platform === platformFilter);
  ...
  const statCards = [
    ...
    },
    {
      ...
      value:
        totalEarnings > 0 ? `${totalEarnings.toLocaleString()} ETB` : "0 ETB",
        ...
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
          <div>
            ...
            <p className="text-gray-500 dark:text-gray-400 text-sm">
              Real-time performance tracking for every post you've submitted
          ...
          <div className="space-y-10">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
              {statCards.map((s, i) => (
                <motion.div
                  ...
                  className="bg-white dark:bg-[#0c0c0c] p-6 rounded-4xl border border-gray-100 dark:border-white/5 shadow-sm"
                >
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
              ...
              <div className="bg-white dark:bg-[#0a0a0a] p-8 rounded-[2.5rem] border border-gray-100 dark:border-white/5 shadow-sm">
                <h3 className="text-lg font-black text-gray-900 dark:text-white mb-8">
                  Platform Reach
                </h3>
                <div className="h-45 w-full mb-6">
                  {completed.length === 0 ? (
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="flex flex-wrap items-center justify-between gap-4 mb-6">
                  <div>
                    ...
                    <p className="text-xs text-gray-400 font-medium mt-0.5">
                      Performance analytics per social media post
                    </p>
                  </div>
                  {/* Platform Filter Pills */}
                  <div className="flex gap-2 flex-wrap">
                    {(["All", "TikTok", "Instagram", "YouTube"] as const).map(
                      (p) => (
                        <button
                          key={p}
                ...
                {filteredPosts.length === 0 ? (
                  <div className="text-center py-12 border border-dashed border-gray-200 dark:border-white/5 rounded-3xl">
                    ...
                    <p className="text-sm font-bold text-gray-400">
                      No {platformFilter !== "All" ? platformFilter : ""} posts
                      tracked yet
                    </p>
                  </div>
                ) : (
              ...
              <div className="space-y-6">
                ...
                <div className="bg-primary-blue rounded-[2.5rem] p-8 text-white shadow-xl shadow-primary-blue/25">
                  ...
                  <div className="space-y-3">
                    <div className="p-4 bg-white/10 rounded-2xl border border-white/10">
                      <p className="text-[9px] font-black text-white/60 uppercase mb-1">
                        Total Views
                      </p>
                ...
                <div className="bg-white dark:bg-[#0a0a0a] border border-gray-100 dark:border-white/5 p-6 rounded-4xl shadow-sm space-y-4">
                  <h4 className="text-xs font-black text-gray-400 dark:text-gray-500 uppercase tracking-[0.2em] flex items-center gap-2">
                    <Briefcase size={13} className="text-primary-blue" /> Posts
                    Breakdown
                  </h4>
                  {(["TikTok", "Instagram", "YouTube"] as const).map((p) => {
                    ...
                    const views = posts
                      ...
                      .reduce((s, r) => s + r.metrics.views, 0);
