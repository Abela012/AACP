...
export default function BalancePage() {
  ...
  const totalEarned = txHistoryData
    ...
    <BusinessLayout>
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-10">
        <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
          ...
          <button 
            onClick={() => navigate('/business/buy-coins')}
            ...
            <div className="bg-emerald-600 p-10 rounded-[2.5rem] text-white relative overflow-hidden shadow-2xl shadow-emerald-100 dark:shadow-none">
              <div className="relative z-10">
                ...
                <div className="flex gap-10">
                  <div>
                    <p className="text-emerald-100 text-[10px] font-bold uppercase tracking-widest mb-1">Total Spent</p>
                    <p className="text-xl font-bold">{totalSpent.toLocaleString()} Coins</p>
                  </div>
                  <div className="w-px h-10 bg-white/20" />
                  <div>
                    <p className="text-emerald-100 text-[10px] font-bold uppercase tracking-widest mb-1">Total Earned</p>
                    <p className="text-xl font-bold">{totalEarned.toLocaleString()} Coins</p>
                  </div>
                </div>
              </div>
              {/* Decorative elements */}
                      ...
                      <div key={t._id} className="p-6 flex items-center justify-between hover:bg-gray-50/50 dark:hover:bg-white/5 transition-colors">
                        <div className="flex items-center gap-4">
                          <div className={cn(
                            ...
                            t.type === 'debit' ? "bg-red-50 dark:bg-red-500/10 text-red-500" : "bg-green-50 dark:bg-green-500/10 text-green-500"
                          )}>
                          ...
                          <div>
                            ...
                            <p className="text-[10px] text-gray-400 dark:text-gray-500 font-medium">{new Date(t.createdAt).toLocaleDateString()}</p>
                          </div>
                        </div>
          ...
          <div className="space-y-8">
                ...
                {coinPackages.map((pkg) => (
                  <div 
                    ...
                    className={cn(
                      ...
                      pkg.popular ? "border-emerald-600 bg-emerald-50/30 dark:bg-emerald-900/20" : "border-gray-50 dark:border-white/5 hover:border-emerald-100 dark:hover:border-emerald-900/30"
                    )}
                  >
                    <div className="flex justify-between items-start mb-4">
                      ...
                      {pkg.popular && (
                        <span className="bg-emerald-600 text-white text-[8px] font-bold px-2 py-1 rounded-lg uppercase tracking-widest">Popular</span>
                      )}
                    </div>
            ...
            <div className="bg-white dark:bg-white/5 p-8 rounded-[2.5rem] border border-gray-100 dark:border-white/5 shadow-sm dark:shadow-none">
              <div className="flex items-center justify-between mb-6">
                ...
                <button className="text-[10px] font-bold text-emerald-600 dark:text-emerald-400 hover:underline">Manage</button>
              </div>
