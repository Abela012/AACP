...
import { Eye, EyeOff, Briefcase, Megaphone, Loader2, Copy, CheckCircle2, AlertTriangle, RefreshCw } from "lucide-react";
...
export default function LoginForm() {
    ...
    const handleTikTokInitiate = async (e: FormEvent) => {
        ...
        try {
            ...
            if (data.loggedInDirectly && data.token) {
                setState('success');
        ...
        <div className="flex flex-col relative w-full max-w-md mx-auto">
            ...
            <div className="mb-8 text-left">
                <h2 className="text-4xl font-bold text-[#001e00] mb-3">
                    {role === 'advertiser' ? 'Sign in with TikTok' : 'Welcome back'}
                </h2>
                <p className="text-base text-[#5e6d55]">
                    {role === 'advertiser' 
                        ? 'TikTok is the primary and only way to sign in. No password required.' 
            ...
            {role === 'business_owner' && (
                <form onSubmit={handleClerkSubmit} className="space-y-6">
                    {infoMessage && (
                        <div className="p-3 bg-emerald-50 border border-emerald-100 text-[#004d2b] text-sm rounded-xl font-medium leading-relaxed">
                            {infoMessage}
                        </div>
                    )}
                    ...
                    {needsVerificationStep ? (
                        <div>
                            ...
                            />
                            <p className="mt-2 text-xs text-[#5e6d55] font-medium leading-relaxed">
                                This is Clerk security (multi-factor authentication or verifying a new device). Use the email code or your authenticator app—then tap Verify Code.
                            </p>
                            <button
                                type="button"
            ...
            <p className="mt-8 text-center text-sm font-medium text-[#5e6d55]">
                {role === 'advertiser' ? 'New to AACP? ' : 'New to AACP? '}
                <Link
                    to="/auth/register"
