      ...
      <Card title="AI readiness" icon={<Sparkles size={18} />}>
        <div className="grid grid-cols-2 gap-4">
          <div>
            ...
            <p className="text-2xl font-black text-emerald-600">{Number(pc.percent) || 0}%</p>
          </div>
          <div>
      ...
      <Card title="Audience summary" icon={<Users size={18} />}>
        <div className="flex flex-wrap gap-1.5 mb-2">
          {((ta.gender as string[]) || []).map((g) => (
            <span key={g} className="px-2 py-0.5 rounded-full bg-emerald-500/10 text-xs font-bold">
              {g}
            </span>
          ))}
